                ...
                @foreach($group as $document)
                    @php
                        ...
                        $associatedPeople = $linkedPeople
                            ...
                            ->unique()
                            ->values();
                        if ($associatedPeople->isEmpty() && isset($person) && $person) {
                            $associatedPeople = collect([trim($person->Nombres . ' ' . $person->Apellidos)]);
                        }
                    @endphp
                    <div class="document-library-card">
                        <button type="button" class="document-library-preview-trigger" data-document-preview-trigger
                            ...
                            <span class="document-library-copy">
                                ...
                                @if($associatedPeople->isNotEmpty())
                                    <em class="document-library-association">Asociado a: {{ $associatedPeople->join(', ') }}</em>
                                @endif
                                <em>{{ optional($document->created_at)->format('d/m/Y') ?: 'Sin fecha' }}</em>
                            </span>
                            <span class="document-library-preview-hint"><i class="far fa-eye" aria-hidden="true"></i> Vista previa</span>
                            @if(! $clientView)
...
<style>
    ...
    .document-library-card { overflow:hidden; border:1px solid #dbe6e9; background:#fff; border-radius:12px; transition:.15s ease; }.document-library-card:hover { border-color:#8eb5be; box-shadow:0 8px 20px rgba(17,72,87,.11); transform:translateY(-1px); }.document-library-preview-trigger { position:relative; display:flex; gap:.7rem; align-items:center; width:100%; border:0; background:#fff; padding:.75rem; text-align:left; cursor:pointer; }.document-library-icon { width:2.3rem; height:2.3rem; border-radius:9px; display:grid; place-items:center; background:#edf2f3; color:#506b74; font-size:1.15rem; flex:none; }.document-library-icon.is-pdf { background:#fff0ef; color:#b9443b; }.document-library-icon.is-image { background:#edf7f1; color:#2d8a5d; }.document-library-copy { min-width:0; display:grid; gap:.12rem; flex:1; }.document-library-copy strong { color:#234854; font-size:.86rem; }.document-library-copy small { color:#637a82; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:150px; }.document-library-copy em { color:#91a0a5; font-size:.7rem; font-style:normal; }.document-library-copy .document-library-association { color:#286270; font-weight:700; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }.document-library-preview-hint { display:inline-flex; align-items:center; gap:.28rem; color:#0b5b6f; font-size:.68rem; font-weight:800; white-space:nowrap; }.document-library-source { position:absolute; right:.55rem; top:.5rem; color:#71858c; font-size:.64rem; text-transform:uppercase; letter-spacing:.04em; }.document-library-associate { width:100%; border:0; border-top:1px solid #e6eef0; padding:.5rem .75rem; text-align:left; background:#f4f9f9; color:#286270; font-size:.78rem; font-weight:700; cursor:pointer; }.document-library-associate:hover { background:#e9f4f4; }

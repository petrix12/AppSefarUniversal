...
<x-app-layout>
                ...
                <div class="bg-gray-50">
                    <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:py-6 lg:px-8 lg:flex lg:items-center lg:justify-between">
                        <h2 class="text-3xl font-extrabold tracking-tight text-gray-900 sm:text-4xl">
                            <span class="ctvSefar block text-indigo-600">Alertas</span>
                        </h2>
                        <div class="mt-8 flex lg:mt-0 lg:flex-shrink-0">
                            <div class="inline-flex rounded-md shadow">
                                <a href="{{ route('alerts.create') }}" class="cfrSefar inline-flex items-center justify-center px-5 py-3 border border-transparent text-base font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700">
                                    Registrar Alerta
                                </a>
                            </div>
                        </div>
    ...
    @if (!is_null($alertas))
        <table class="min-w-full divide-y divide-gray-200 w-100" id="example">
            ...
            <tr>
                <td>{{ $alerta->title }}</td>
                <td>{{ date("d-m-Y", strtotime($alerta->start_date)) }}</td>
                <td>{{ date("d-m-Y", strtotime($alerta->end_date)) }}</td>
                <td>
                    <a href="#" onclick="previewImage('{{$alerta->image}}')">
                ...
                <td class="text-center">
                    <form action="{{ route('alerts.destroy', $alerta) }}" method="POST">
                        @csrf
                        @method('delete')
                        <button type="submit" class="text-red-600 hover:text-red-900" onclick="return confirm('¿Está seguro que desea eliminar esta alerta?')">

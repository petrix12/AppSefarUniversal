...
<form method="POST" action="{{ route('invoices.update', $invoice) }}" id="invoice-form">
                ...
                <div class="card-body">
                    ...
                    <div class="form-group">
                        ...
                        <select name="payment_terms" class="form-control">
                            ...
                            <option value="immediate" {{ old('payment_terms',$invoice->payment_terms)==='immediate'?'selected':'' }}>Inmediato</option>
                            <option value="14_days"   {{ old('payment_terms',$invoice->payment_terms)==='14_days'  ?'selected':'' }}>14 días</option>
                            <option value="30_days"   {{ old('payment_terms',$invoice->payment_terms)==='30_days'  ?'selected':'' }}>30 días</option>
                            <option value="60_days"   {{ old('payment_terms',$invoice->payment_terms)==='60_days'  ?'selected':'' }}>60 días</option>
                            <option value="90_days"   {{ old('payment_terms',$invoice->payment_terms)==='90_days'  ?'selected':'' }}>90 días</option>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="payment_method" class="form-control">
                            ...
                            <option value="bizum"    {{ old('payment_method',$invoice->payment_method)==='bizum'   ?'selected':'' }}>Bizum</option>
                    ...
                    <div class="form-group">
                        ...
                        <select name="bank_account" class="form-control">
                            <option value="standard" {{ old('bank_account',$invoice->bank_account)==='standard'?'selected':'' }}>Estándar</option>
                            <option value="other"    {{ old('bank_account',$invoice->bank_account)==='other'   ?'selected':'' }}>Otra</option>
                        </select>
                    </div>
            ...
            <div class="card card-warning card-outline">
                ...
                <div class="card-body">
                    ...
                    <div class="form-group">
                        ...
                        <select name="captador_id" class="form-control">
                            ...
                            @foreach($captadores as $c)
                                <option value="{{ $c->id }}"
                                    {{ old('captador_id', $invoice->captador_id) == $c->id ? 'selected' : '' }}>
                                    {{ $c->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="sales_team"
                               value="{{ old('sales_team', $invoice->sales_team) }}"
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="send_email"
                               value="{{ old('send_email', $invoice->send_email) }}"
                    ...
                    <div class="form-group">
                        ...
                        <select name="product_service" class="form-control">
                            <option value="">— Elegir —</option>
                            <option value="española_lmd"       {{ old('product_service',$invoice->product_service)==='española_lmd'       ?'selected':'' }}>Española LMD</option>
                            <option value="española_sefardi"   {{ old('product_service',$invoice->product_service)==='española_sefardi'   ?'selected':'' }}>Española Sefardí</option>
                            <option value="portuguesa_sefardi" {{ old('product_service',$invoice->product_service)==='portuguesa_sefardi' ?'selected':'' }}>Portuguesa Sefardí</option>
                            <option value="italiana"           {{ old('product_service',$invoice->product_service)==='italiana'           ?'selected':'' }}>Italiana</option>
                            <option value="gestion_documental" {{ old('product_service',$invoice->product_service)==='gestion_documental' ?'selected':'' }}>Gestión Documental</option>
                            <option value="otros"              {{ old('product_service',$invoice->product_service)==='otros'              ?'selected':'' }}>Otros</option>
                        </select>
                    </div>
                ...
                <div class="card-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="deposit_number_client"
                               value="{{ old('deposit_number_client', $invoice->deposit_number_client) }}"
                               class="form-control">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="deposit_number_sefar"
                               value="{{ old('deposit_number_sefar', $invoice->deposit_number_sefar) }}"
                               class="form-control">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="paid_by"
                               value="{{ old('paid_by', $invoice->paid_by) }}"
                               class="form-control">
                    </div>
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <form method="POST" action="{{ route('invoices.destroy', $invoice) }}"
              onsubmit="return confirm('¿Eliminar esta factura? No se puede deshacer.')">
            @csrf
            @method('DELETE')

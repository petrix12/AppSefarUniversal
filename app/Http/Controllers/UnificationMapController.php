...
class UnificationMapController extends Controller
{
    ...
    public function suggest(
        ...
        $batchCandidates = $this->selectedBatchCandidates($data, $inventory['field_options']);
        ...
        $selectedCandidate = is_array($batchCandidates)
            ? null
            : $this->selectedPairCandidate($data, $inventory['field_options']);
        ...
        $candidates = is_array($batchCandidates)
            ...
            ? [$selectedCandidate]
            : $this->automaticCandidatesForProviders($data, $inventory['automatic_relations']));
    ...
    public function storeRelationsBulk(Request $request, UnificationMapAuditService $map): JsonResponse
    {
        ...
        $fieldOptions = $map->inventory()['field_options'];
        $records = [];
        $seen = [];
        $skippedExisting = 0;
    ...
    private function canAuditEntityTypesBeMapped(string $leftType, string $rightType): bool
    {
        ...
        if ($leftFamily === $rightFamily) {
            return in_array($leftFamily, ['contact', 'commercial'], true);
        }
    ...
    private function selectedEndpoint(string $side, array $data, array $fieldOptions): ?array
    {
        ...
        return $this->endpointForField($provider, $entityType, $scopeKey, $fieldKey, $fieldOptions);
    }
    ...
    private function endpointForField(string $provider, string $entityType, string $scopeKey, string $fieldKey, array $fieldOptions): ?array
    {

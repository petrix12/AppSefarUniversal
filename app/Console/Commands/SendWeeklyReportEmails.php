...
class SendWeeklyReportEmails extends Command
{
     ...
     * @var string
     */
    protected $signature = 'send:weklyreport';
    ...
    ->where('name', 'not like', '%prueba%')
        ...
        $chartUrl = mostrarGraficoQuickChartSemanal('https://quickchart.io/chart?c=' . urlencode(json_encode($chartConfig)));
        ...
        $pdf = PDF::loadView('reportes.plantillasemanal', compact(
            'peticion',
            'usuariosHoy',
        ...
        Mail::send('mail.reporte-weekly', compact(
            ...
            'fechaInicioFormato',
            'fechaFinFormato',
        ), function ($message) use ($pdfContent, $peticion, $fechaInicio, $fechaFin, $emails) {
            $message->to($emails)
        ...
        try {
            ...
            $fileName = 'reporte_semanal_' . Carbon::parse($fechaInicio)->format('d/m/Y') . '-' . Carbon::parse($fechaFin)->format('d/m/Y') . '.pdf';
            ...
            foreach ($numbers as $number) {
                $response = Http::timeout(30)->post(WhatsappBotURL::findOrFail(1) . '/send-file', [
                    ...
                    'message' => "📊 *Reporte Semanal*\n\nFecha: ".Carbon::parse($fechaInicio)->format('d/m/Y')." al " . Carbon::parse($fechaFin)->format('d/m/Y'),
                    'fileData' => $pdfBase64,
                    'fileName' => $fileName
                ]);
            }
...
function mostrarGraficoQuickChartSemanal($chartUrl, $mimeType = 'image/png') {
